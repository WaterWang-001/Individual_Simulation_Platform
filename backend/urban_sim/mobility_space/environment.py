...
class AoiResult(BaseModel):
    ...
    id: int = Field(..., description="AOI ID — pass this to move_to()")
    name: str = Field(..., description="AOI name")
    lnglat: Optional[List[float]] = Field(None, description="[longitude, latitude] of AOI centroid")
    distance: Optional[float] = Field(None, description="Distance from search center in meters")


class FindAoisResponse(BaseModel):
    """Response model for AOI search functions"""

    aois: List[AoiResult] = Field(..., description="List of matching AOIs")
...
"""
    ...
    async def init(self, start_datetime: datetime) -> Any:
        ...
        get_logger().info("MobilitySpace initialized with straight-line routing.")
    ...
    async def _write_position(
        ...
        """No-op: replay writer removed in lightweight version."""
        pass
    ...
    async def move_to(
        ...
        # Straight-line routing: start → destination
        start_xy = person.position.xy
        ...
        if aoi_id_or_poi_id >= POI_START_ID:
            ...
            if poi is None:
                return
            dest_xy = (poi["position"]["x"], poi["position"]["y"])
            dest_aoi_id_resolved = poi["aoi_id"]
            dest_poi_id_resolved = aoi_id_or_poi_id
        else:
            aoi = self._map.get_aoi(aoi_id_or_poi_id)
            if aoi is None:
                return
            dest_xy = (aoi["shapely_xy"].centroid.x, aoi["shapely_xy"].centroid.y)
            dest_aoi_id_resolved = aoi_id_or_poi_id
            dest_poi_id_resolved = None

        path = LineString([start_xy, dest_xy])
        speed = WALKING_SPEED if mode == "walking" else DRIVING_SPEED
        path_v = speed
        ...
        target_position = Position(
            kind="aoi",
            aoi_id=dest_aoi_id_resolved,
            poi_id=dest_poi_id_resolved,
            xy=dest_xy,
            lnglat=self._map.projector(dest_xy[0], dest_xy[1], inverse=True),
        )
    ...
    @tool(readonly=True)
    async def search_aoi_by_name(self, keyword: str, limit: int = 8) -> FindAoisResponse:
        """
        Search for Areas of Interest (AOIs) by name keyword.
        Use this to find the AOI ID of a named place (e.g. a university, library, park, mall)
        before calling move_to().

        Args:
            keyword: Chinese or English keyword to search for (e.g. '复旦大学', '图书馆', 'Fudan')
            limit: Maximum number of results to return (default 8)

        Returns:
            List of matching AOIs with their IDs, names, and coordinates.
            Use the returned id directly in move_to().
        """
        keyword_lower = keyword.lower()
        results = []
        for aoi in self._map._aoi_list:
    ...
    @tool(readonly=True)
    async def find_nearby_aois(
        self, x: float, y: float, radius: float, limit: int = 8
    ) -> FindAoisResponse:
        """
        Find named Areas of Interest (AOIs) near a location.

        Args:
            x: Longitude of the search center
            y: Latitude of the search center
            radius: Search radius in meters (e.g. 1000 for 1 km)
            limit: Maximum number of results (default 8)

        Returns:
            List of nearby named AOIs with IDs, names, and distances.
